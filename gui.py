...
def save_file():
    file_path = tkinter.filedialog.asksaveasfilename(title='Save a file', defaultextension=".docx",
...
def payloads():
    ...
    payload_0 = []
    for j in r:
        str = payload[j].replace(" ","\\ ")
        str = str.replace("\"","\\\"")
        payload_0.append(str)
    payload_1 = [" And\ 0;-- ",
                 "/**/And/**/0;#",
                 "\tAnd\t0;#",
                 "\nAnd/**/0;#",
...
scan = tk.Button(root, text="Scan",
                 command=lambda: thread(vulscan, url.get(), int(threads.get()), int(depth.get()), "sql", output,
                                        open("log.txt", 'a'), waf))
